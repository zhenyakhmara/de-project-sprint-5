...
def load_deliveries (pg_table, pg_schema):
    limit = 50
    offset = 0
    response_len = 21
   ...
   # Удаляем записи за сегодня, чтобы их перегрузить в случае ошибки
    date_for_delete = "'" + (str(date.today())+' 00:00:00') + "'"
    ...
    while response_len > 20:
        url = f"""https://d5d04q7d963eapoepsqr.apigw.yandexcloud.net/deliveries?from={(str(date.today())+' 00:00:00')}&to={(str(date.today())+' 23:59:59')}&sort_field=_id&sort_direction=asc&limit={limit}&offset={offset}"""
        r = requests.get (url, headers=headers)
        offset += 50
        response_len = len(r.text)
        if response_len < 20:
